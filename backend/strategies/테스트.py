def generate_signals(data):
    ...
    ma120 = data['Close'].rolling(window=120, min_periods=1).mean()

    # 1. 상승 추세 조건: MA20 > MA60
    uptrend = ma20 > ma60

    # 2. 눌림목 및 120일 이평선 조건: 이전 봉에서 종가가 MA5 아래(조정), 이번 봉에서 다시 MA5 상향돌파(반등), 그리고 현재 종가가 MA120 위
    close_prev = data['Close'].shift(1)
    ma5_prev = ma5.shift(1)
    pullback_condition = (close_prev < ma5_prev) & (data['Close'] > ma5) & (data['Close'] > ma120) & uptrend
    ...
    signals[:max(5, 20, 60, 120)-1] = 'hold'
