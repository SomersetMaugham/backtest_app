...
PORT=5001
busy=$(lsof -i :$PORT -sTCP:LISTEN -t || true)

if [ -n "$busy" ]; then
    echo "→ Port $PORT is busy (PID $busy). Stopping it..."
    kill "$busy"
    # 혹시 바로 안 죽으면 조금 기다렸다 한 번 더 확인
    sleep 1
fi
...
FLASK_PID=$!
# 로그 바로 보기
#tail --pid=$FLASK_PID -n 20 -f "$BACKEND_DIR/flask.log"
